...
先看f1,f2，都有一个_proto_属性。通过查阅资料得知，所有对象，都有一个隐式引用（非开发者创建），[[prototype]]，chrome浏览器中用_proto_表示这个属性，以下都沿用这一命名。  
...
Object.getOwnPropertyNames() 遍历自身所有属性，不管是否可枚举。    
...
function create() {
    ...
    obj.setPrototypeOf(Con.prototype);
