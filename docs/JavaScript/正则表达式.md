...
```
### 创建RegExp对象的方法，需要对特殊字符做转义处理
```js
var reg = new RegExp('\\d'); // \d表示匹配数字，在这之前要加转义处理
reg.test('jk'); // false
reg.test('12'); // true
...
开头记录了一句话，正则匹配的是字符或者位置，这里总结一下匹配位置的特殊字符。  
6个字符匹配位置
^    
$       
\b         
...
平常用到正则基本都应用于字符串，所以只列举对字符串常用的正则有关的方法，其他应用于正则表达式的方法不再赘述。
