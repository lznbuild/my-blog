...
有个问题，如果一个资源在缓存时间已过了，但是资源没有被修改过，这样的资源会再一次从后端发送到前端，这也是一种浪费。  
...
如果没有更新，就返回 304 状态码，相当于服务器告诉浏览器：“这个缓存可以继续使用，这次就不重复发送数据给你了，也就是协商缓存策略。
...
协商缓存：协商缓存命中，服务器会将这个请求返回304,通过Last-Modified（后端给前端）/If-Mondified-since（前端给后端）   ETag（后端给前端）   If-None-Match（前端给后端） 控制
