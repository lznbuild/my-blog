...
## es6中的class中只有static修饰符(不能在constructor中写，且不会绑定到this)，没有public,private,protected,readonly，static修饰的属性会被继承，不会被实例访问,不可修改
加typescript后会有这些修饰符
...
## sessionStorage 
sessionStorage是会话级别的存储方式，不能在所有同源窗口中共享。 
