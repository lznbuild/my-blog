...
7.webpack。IgnorePlugin 忽略指定的模块，不把指定的模块打包（moment.js里有大量的i18n代码）
```js
plugins: [
...
使用这个可以配合 IgnorePlugin 来过滤掉部分大而无用的第三方模块,BundleAnalyzerPlugin 将各个包的内容，信息以图形化界面展现出来
...
tree shaking(去除没有引用的代码,css的，js的)和uglifyjs(代码压缩混淆)
